...
class SwaggerValidatorRule() : Rule() {
    ...
    override fun visitClass(klass: KtClass) {
        ...
        if (klass.annotationEntries.isNotEmpty()) {
            ...
            val swaggerDefinition = SwaggerDownloader.instance(config = ruleSetConfig).getFuture().get()
                .definitions.find { it.name == swaggerModelName }
            ...
            swaggerDefinition.properties.forEach { swaggerProperty ->
                val modelParameter =
                    modelConstructorParameters.find { parameter -> parameter.name == swaggerProperty.name }
        ...
        when (swaggerProperty.type.type) {
            SwaggerPropertyTypes.STRING -> if (swaggerProperty.type.enumValues != null) {
                if (modelParameter.type.name == "String") {//This is allowed, but could be enum
                    ...
                    PostValidationHelper.markRequiredEnum(
                        modelParameter.type.name,
                        this,
                        modelParameter.element,
                        swaggerProperty.type.enumValues
                    )
                }
            } else {
                when (swaggerProperty.type.format) {
                    "date-time", "date" -> reportTypeMismatch(
                        modelParameter.element,
                        listOf("String", "Date", "Calendar"),
                        modelParameter.type.name
                    )
                    "byte" -> reportTypeMismatch(
                        modelParameter.element,
            ...
            SwaggerPropertyTypes.INTEGER -> when (swaggerProperty.type.format) {
                "int64" -> reportTypeMismatch(modelParameter.element, "Long", modelParameter.type.name)
                "integer", "int32", null -> reportTypeMismatch(
                    modelParameter.element,
                    listOf("Int", "Long"),
                    modelParameter.type.name
                )
                else -> reportTypeMismatch(
                    modelParameter.element,
                    "<error, unknown swagger format for integer>",
                    modelParameter.type.name
                )
            }
            SwaggerPropertyTypes.BOOLEAN -> reportTypeMismatch(
                modelParameter.element,
                "Boolean",
                modelParameter.type.name
            )
            ...
            SwaggerPropertyTypes.REF -> {
                PostValidationHelper.markRequiredDeclarationValidation(
                    swaggerProperty.type.referredType!!,
                    this,
                    modelParameter.element
                )
            }
    ...
    private fun getModelConstructorParameters(constructor: KtPrimaryConstructor): List<ConstructorParameter> {
        return constructor.valueParameters.map { parameter ->
            ...
            val jsonAnnotation =
                parameter.annotationEntries.find { annotation -> annotation.shortName?.asString() == "Json" }
            val overrideName = jsonAnnotation?.let { overrideAnnotation ->
                extractStringFieldFromAnnotation(overrideAnnotation, "name") {
                    throw IllegalStateException(
                        "Got json annotation but not name field! (${Entity.from(
                            overrideAnnotation.navigationElement
                        ).compact()})"
                    )
                }
            }
        ...
        fun create(typeElement: KtTypeElement): Type {
            ...
            if (name == "ParcelableStringList") {
                return Type(
                    ...
                    isNullable,
                    true,
                    listOf(Type("String", isNullable = false, isGeneric = false, genericParameters = emptyList()))
                )
            }
...
private fun extractStringFieldFromAnnotation(
    annotation: KtAnnotationEntry,
    annotationFieldName: String,
    messageIfNotString: () -> Unit
): String? {
...
private fun buildNamedEntity(source: PsiElement, overrideName: String): Entity {
    return Entity.from(source).copy(name = overrideName)
}